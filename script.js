document.addEventListener("DOMContentLoaded", function () {
  ...
  function total() { return quoteItems.reduce((sum, item) => sum + item.unitPrice * item.qty, 0); }

  function sameConfig(a, name, price, addons) {
    if (a.name !== name || a.basePrice !== price) return false;
    const aNames = (a.addons || []).map(x => x.name).sort();
    const bNames = (addons || []).map(x => x.name).sort();
    return aNames.length === bNames.length && aNames.every((x, i) => x === bNames[i]);
  }
  ...
  function renderQuote() {
    if (!quoteItems.length) {
      ...
      quoteItemsEl.innerHTML = quoteItems.map((item, index) => {
        const addons = (item.addons || []).map(a => `${a.name} +₹${money(a.price)}`).join(" · ");
        return `
          <div class="quote-line">
            <div>
              <strong>${item.name}</strong>
              <small>₹${money(item.basePrice)} base${addons ? " · " + addons : ""}</small>
            </div>
  ...
  document.querySelectorAll(".add-service").forEach(button => {
    button.addEventListener("click", function () {
      ...
      const addonInputs = itemBody ? [...itemBody.querySelectorAll(".service-addon:checked")] : [];
      const addons = addonInputs.map(input => ({
        name: input.dataset.addon,
        price: Number(input.dataset.price || 0)
      }));
      const addonTotal = addons.reduce((sum, addon) => sum + addon.price, 0);
      const unitPrice = basePrice + addonTotal;
      const existing = quoteItems.find(item => sameConfig(item, name, basePrice, addons));

      if (existing) existing.qty += qty;
      else quoteItems.push({ name, basePrice, addons, unitPrice, qty });

      renderQuote();
      button.textContent = "Added ✓";
      setTimeout(() => button.textContent = "Add to quotation", 1000);
    });
  });

  function openModal() {
    if (!quoteItems.length) return;
    const itemText = quoteItems.map(i => { const extras = (i.addons || []).map(a => `${a.name} +₹${money(a.price)}`).join(", "); return `${i.name} × ${i.qty}${extras ? ` [${extras}]` : ""} — ₹${money(i.unitPrice * i.qty)}`; }).join("\n");
    ...
    document.getElementById("quoteConditions").value = "Estimate based on selected starting prices. Final price confirmed after requirements are reviewed.";
  ...
  form.addEventListener("submit", async function (event) {
    ...
    button.disabled = true; button.textContent = "SENDING..."; status.textContent = "Sending your quotation..."; status.className = "form-status";
    const data = Object.fromEntries(new FormData(form).entries());
    try {
      await emailjs.send(SERVICE_ID, ADMIN_TEMPLATE, data);
      await emailjs.send(SERVICE_ID, CUSTOMER_TEMPLATE, data);
      status.textContent = "Quotation sent successfully! Check your email.";
      ...
      status.textContent = "Email could not be sent. Please try again.";
      status.className = "form-status error";
    } finally {
      button.disabled = false; button.textContent = "Send Quotation Request →";
    }
  });
