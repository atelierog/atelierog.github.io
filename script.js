document.addEventListener("DOMContentLoaded", function () {
  ...
  function total() { return quoteItems.reduce((sum, item) => sum + item.price * item.qty, 0); }
    ...
    if (!quoteItems.length) {
      ...
      quoteItemsEl.innerHTML = quoteItems.map((item, index) => `
        <div class="quote-line">
          ...
          <div class="quote-controls"><button type="button" data-minus="${index}">−</button><span>${item.qty}</span><button type="button" data-plus="${index}">+</button><b>₹${money(item.price * item.qty)}</b><button class="remove-item" type="button" data-remove="${index}" aria-label="Remove">×</button></div>
        </div>`).join("");
  ...
  document.querySelectorAll(".add-service").forEach(button => {
    button.addEventListener("click", function () {
      ...
      const existing = quoteItems.find(item => item.name === name && item.price === price);
      if (existing) existing.qty += qty;
      else quoteItems.push({ name, price, qty });
      renderQuote();
  ...
  function openModal() {
    ...
    const itemText = quoteItems.map(i => `${i.name} × ${i.qty} — ₹${money(i.price * i.qty)}`).join("\n");
